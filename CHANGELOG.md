...
## [0.2.x] — prior unreleased additions
...
- **`PROMOTION.md`** — practical virality playbook: positioning, channels, sample
  posts (HN / Reddit / Twitter), launch checklist, comparison hooks vs Manim /
  Lottie / Motion Canvas, SEO keywords, content-series ideas, metrics.

### Changed
- Workspace version bumped `0.1.0` → `0.2.0` to match README/CHANGELOG.
- `Renderer` trait gains `load_image` (defaulted no-op) and `set_time` (defaulted
  no-op), so existing `VelloRenderer` and test stubs compile unchanged.
- WASM `hit_test` / `get_z_index` extended to cover all 17 object types.
- `lumina-server` CI gains mdBook build + GitHub Pages deploy job.
...
- **Vello GPU backend** — full `VelloRenderer` with wgpu CPU software fallback.
