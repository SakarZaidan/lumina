...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok(scene) = serde_json::from_str::<lumina_schema::Scene>(text) else {
    ...
    let response = lumina_core::validation::validate_scene_data(&scene);
    ...
    if response.valid {
        let _ = lumina_core::Timeline::from_scene(&scene);
        let _ = lumina_core::SceneGraph::from_scene(&scene);
    }
});
