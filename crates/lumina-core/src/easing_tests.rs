...
mod tests {
    ...
    macro_rules! test_boundary {
        ($fn:ident) => {
            assert!(approx_eq($fn(0.0), 0.0), "{} at 0.0 should be 0.0, got {}", stringify!($fn), $fn(0.0));
            assert!(approx_eq($fn(1.0), 1.0), "{} at 1.0 should be 1.0, got {}", stringify!($fn), $fn(1.0));
        };
    }
    ...
    fn test_ease_in_is_slower_than_linear_at_start() {
        ...
        let t = 0.3_f32;
        assert!(ease_in_quad(t) < t, "ease_in_quad should accelerate (below linear at {t})");
    ...
    fn test_ease_out_is_faster_than_linear_at_start() {
        ...
        let t = 0.3_f32;
        assert!(ease_out_quad(t) > t, "ease_out_quad should decelerate (above linear at {t})");
    ...
    fn test_elastic_out_overshoots_then_settles() {
        ...
        let peak = (1..20).map(|i| ease_out_elastic(i as f32 / 20.0)).fold(f32::NEG_INFINITY, f32::max);
        assert!(peak > 1.0, "ease_out_elastic should overshoot 1.0, peak was {peak}");
    ...
    fn test_spring_starts_at_zero_ends_near_one() {
        ...
        let end = spring(1.0);
        assert!((end - 1.0).abs() < 0.05, "spring(1.0) should be near 1.0, got {end}");
    }
    ...
    fn test_there_and_back_midpoint_is_one() {
        ...
        assert!(approx_eq(peak, 1.0), "there_and_back(0.5) should be 1.0, got {peak}");
    ...
    fn test_unknown_easing_falls_back_to_linear() {
        ...
        for i in 0..=5 {
            let t = i as f32 / 5.0;
            assert!(approx_eq(f(t), linear(t)), "unknown easing should behave like linear at t={t}");
        }
    }
    ...
    fn test_in_out_variants_are_symmetric() {
        ...
        for f in fns {
            for i in 0..=10 {
                let t = i as f32 / 10.0;
                let sum = f(t) + f(1.0 - t);
                assert!((sum - 1.0).abs() < 1e-4, "in_out symmetry violated at t={t}: f(t)+f(1-t)={sum}");
            }
        }
    }
}
