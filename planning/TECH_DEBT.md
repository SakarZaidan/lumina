...
| TD-07 | Untyped `serde_json::Value` properties silently default on typos | High | Authoring correctness; breaking schema change | v0.6 / v0.7 | **Stage 1 done** (RFC-0002, ADR-0015) — unknown and mistyped properties are validation errors with a did-you-mean, read from the structs via `schemars`. Its first run found a live instance in shipped examples: `font_id` on LaTeX and MathML was always dropped. **Stage 2 open** — typed render state, so a misspelling *inside* the engine stops compiling and defaults exist once (216 string reads remain) |
...
| TD-12 | JS SDK unbuildable (`../wasm/` import missing, no wasm-pack wiring); `node_modules` committed | High | JS SDK unusable from clean checkout | v0.5 | **Closed** — `build:wasm` generates the module and `prepack` runs it, so a clean checkout builds and `npm pack` always carries the wasm. Fixing it exposed a second, quieter failure: `wasm-pack` writes a `.gitignore` containing `*` into its output directory, and npm honours it, so the tarball shipped with none of the WebAssembly while `npm pack` reported success. A CI job now inspects the tarball's contents, which is the only thing that catches either |
...
| TD-27 | Every `*Props` struct is exhaustively constructible, so **adding any property to any object type is a semver-major change** for Rust API consumers | Medium | Nearly every future schema addition — Stage 2 of RFC-0002, TD-26's no-fill, new object features — forces a major bump on `luminafx-schema` and everything above it | before v1.0 | Open — found when `cargo-semver-checks` (#98) failed #107 for adding `font_id` to `LaTeXProps`, and the bench's own `LaTeXProps { … }` literal stopped compiling as proof. Candidates: `#[non_exhaustive]` plus a construction story (a builder, or `Default` where every field has one). That change is itself breaking, so it wants making once, deliberately, rather than each time a property is added |
