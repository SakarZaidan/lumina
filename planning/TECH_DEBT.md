...
| TD-10 | Zero tests: `lumina-text`, `lumina-schema`, `lumina-cli` | Medium | Font fallback, serde contract, CLI regressions | v0.6 | Partly addressed ([#69](https://github.com/SakarZaidan/lumina/pull/69) — `lumina-text` has its first tests). `lumina-schema` and `lumina-cli` still have none |
