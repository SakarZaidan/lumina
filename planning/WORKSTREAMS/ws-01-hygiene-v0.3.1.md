...
- [ ] `planning/` system: README, ROADMAP (absorbs todo.md), STATUS,
      KNOWLEDGE_BASE, TECH_DEBT, DECISIONS, WORKSTREAMS, AI prompts;
      move blueprint + history.md under planning/
- [ ] Cargo metadata: canonical repo URL (D-001), authors, per-crate
      description/keywords/categories/readme, `publish = false` on
      server+wasm (D-009); pyproject 0.2.0→0.3.0; MSRV probe
- [ ] Crate-level `//!` rustdoc, all crates + CLI
- [ ] README: live CI badge, easing-count fix, Pages URL, canonical links
- [ ] CHANGELOG: dedupe [0.1.0], fold [0.2.x] into [0.3.0], compare links
- [ ] CONTRIBUTING unified; SECURITY.md, CODE_OF_CONDUCT.md, CODEOWNERS,
      dependabot.yml, issue-template config.yml
- [ ] examples/README.md with portability notes
- [ ] mdBook v0.3.0 refresh: events chapter, flag updates, parity table;
      legacy ARCHITECTURE.md/SPEC.md → pointer stubs (D-006)
- [ ] PR → green CI on main → merge
- [ ] Tags v0.1.0 (9348d97, backdated), v0.2.0 (02b92da, backdated),
